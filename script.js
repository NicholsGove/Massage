...
const music = (function () {
  ...
  const N = 7;
  const TRACKS = Array.from({ length: N }, (_, i) => `music/song${i + 1}.mp3`);
  let idx = 0, started = false, fails = 0, isPlaying = false;
  ...
  function play() {
    ...
    const p = audio.play();
    if (p && p.catch) p.catch(() => attention()); // autoplay blocked -> invite a tap
  }

  audio.addEventListener("playing", () => {
    fails = 0; isPlaying = true;
    muteBtn.classList.add("playing");
    muteBtn.classList.remove("attention");
  });
  audio.addEventListener("pause", () => { isPlaying = false; muteBtn.classList.remove("playing"); });

  // advance to next song when one finishes
  audio.addEventListener("ended", () => { idx = (idx + 1) % N; fails = 0; play(); });

  // if a file can't load/decode, skip to the next one (don't loop forever)
  audio.addEventListener("error", () => {
    if (!started) return;
    fails++;
    if (fails >= N) { console.warn("[music] None of song1.mp3 … song7.mp3 could play. Run the diagnostic below."); diagnose(); return; }
    idx = (idx + 1) % N;
    play();
  });

  function attention() {
    // draw the eye to the ♪ button so a single tap starts the music
  ...
  function start() {
    ...
    muteBtn.classList.remove("hidden");
    play();
    // if nothing is playing shortly after the tap, the browser likely blocked it
    setTimeout(() => { if (!isPlaying && audio.paused) attention(); }, 1500);
  }

  muteBtn.addEventListener("click", () => {
    muteBtn.classList.remove("attention");
    if (audio.paused) {            // not started / blocked -> start it
      audio.muted = false;
    ...
    audio.muted = !audio.muted;     // already playing -> just toggle mute
    muteBtn.classList.toggle("muted", audio.muted);
    muteBtn.textContent = audio.muted ? "🔇" : "♪";
  });

  // Console diagnostic: tells you if files are missing (404) or are Git LFS pointers (tiny size)
  async function diagnose() {
    console.log("%c[music] checking /music files…", "color:#b8607c;font-weight:bold");
    for (let i = 1; i <= N; i++) {
      const url = `music/song${i}.mp3`;
      try {
        const r = await fetch(url, { method: "GET", headers: { Range: "bytes=0-1023" } });
        const size = r.headers.get("content-range") || r.headers.get("content-length") || "?";
        let note = "";
        const bytes = parseInt((r.headers.get("content-range") || "").split("/")[1] || r.headers.get("content-length") || "0", 10);
        if (r.status === 404) note = "  <-- NOT FOUND (check the name/case)";
        else if (bytes && bytes < 1000) note = "  <-- looks like a Git LFS pointer, not a real mp3";
        console.log(`song${i}.mp3 : HTTP ${r.status}, size ${size}${note}`);
      } catch (e) {
        console.log(`song${i}.mp3 : fetch failed`, e);
      }
    }
  }
